...
public:
    ...
    AirportCounter(TIterator begin, TIterator end) 
        : AirportCounter()
    {
        for (auto airport = begin; airport != end; ++airport) {
            Insert(*airport);
        }
    }

    size_t Get(TAirport airport) const {
        size_t airport_n = static_cast<size_t>(airport);
        return airports_[airport_n];
    }
    ...
    void EraseAll(TAirport airport) {
        size_t airport_n = static_cast<size_t>(airport);
        airports_[airport_n] = 0;
    }

    using Item = pair<TAirport, size_t>;
    using Items = array<Item, static_cast<size_t>(TAirport::Last_)>; /* FIXME: ??? */
...
private:
    array<int, static_cast<size_t>(TAirport::Last_)> airports_; /* FIXME: ??? */

    void Print() {
        for (size_t i = 0; i < static_cast<size_t>(TAirport::Last_); ++i) {
            cout << "airport: " << i << "; flights: " << airports_[i] << "\n";
        }
    }
};
