...
struct AddressHasher {
    size_t operator() (const Address& address) const {
        ...
        size_t h_city = hash_string(address.city);
        size_t h_street = hash_string(address.street);
        size_t h_building = hash_int(address.building);

        return prime * prime * h_city + prime * h_street + h_building;
    }
...
struct PersonHasher {
    size_t operator() (const Person& p) const {
        ...
        size_t h_name = hash_string(p.name);
        size_t h_height = hash_int(p.height);
        size_t h_weight = hash_double(p.weight);
        size_t h_address = hash_address(p.address);

        return prime * prime * prime * h_name + prime * prime * h_height + prime * h_weight + h_address;
    }
